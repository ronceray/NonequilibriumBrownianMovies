...
def Plot_Sdot_Perc_Modes(results,output_dir):
	...
	plt.tight_layout()
	f.savefig(output_dir+"Sdot_percentage_"+results['Basis_Type'].iloc[0]+f"_Force_order_{results['Force_Basis'].iloc[0]}_Diff_order_{results['Diff_Basis'].iloc[0]}2.png",dpi=400)
	plt.close()
...
def Plot_Components(Analysis_Type,output_dir):
	...
	projections=np.load(output_dir+f"{Analysis_Type}_projections_test.npy")
	...
	plt.imshow(components[0].reshape(30,30),cmap='RdGy_r',vmin=vmin,vmax=vmax)
	ax2.set_xticks([])
	ax2.set_yticks([])
	...
	plt.imshow(components[1].reshape(30,30),cmap='RdGy_r',vmin=vmin,vmax=vmax)
	ax4.set_xticks([])
	ax4.set_yticks([])
	...
	plt.imshow(components[2].reshape(30,30),cmap='RdGy_r',vmin=vmin,vmax=vmax)
	ax6.set_xticks([])
	ax6.set_yticks([])
	...
	plt.imshow(components[3].reshape(30,30),cmap='RdGy_r',vmin=vmin,vmax=vmax)
	ax8.set_xticks([])
	ax8.set_yticks([])
