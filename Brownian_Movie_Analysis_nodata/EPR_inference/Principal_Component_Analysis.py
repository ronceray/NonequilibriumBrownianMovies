...
def PCA(input_directory,output_directory,truncation_criteria=True):
  ...
  X=np.load(input_directory+"Images/images.npy") #Loads brownian movie (rows=time, columns=pixel values)
  ...
  PCA_projections_all=(X).dot(PCA_components[:max_modes].T) #Projects the full dataset onto the PCs
  PCA_projections_test=PCA_projections_all[:X_test.shape[0]] #Projected test set
  print('Done, saving...')
  np.save(output_directory+'PCA_components.npy', PCA_components[:max_modes])
  np.save(output_directory+'PCA_projections_all.npy', PCA_projections_all[:,:max_modes])
  ...
  if(truncation_criteria==True):
    ...
    for i in range(max_modes):
        autocorrelation_function=acf(PCA_projections[:1000,i],nlags=2) #Computes first 10 values of autocorrelation function.
    ...
    plt.xlabel(r"mode $i$")
    plt.ylabel(r"$\langle (\Delta c_i)^2 \rangle/\langle c_i^2 \rangle$")
    plt.tight_layout()
    fig.savefig(output_directory+"2nd_truncation_criterion.png")
  return PCA_projections_all
